...
The staging and production definitions deploy prebuilt backend and frontend images from Docker Hub. Staging uses the `alpha` tags and production uses the `latest` tags. The gateway is the only externally exposed application service; its port can be overridden with `FRONTEND_PORT`.
