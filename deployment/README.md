...
The staging and production definitions deploy prebuilt images. Set `AI_SERVICE_BACKEND_IMAGE` and `AI_SERVICE_FRONTEND_IMAGE` to immutable image references before using them. The exposed ports can be overridden with `BACKEND_PORT` and `FRONTEND_PORT`.
