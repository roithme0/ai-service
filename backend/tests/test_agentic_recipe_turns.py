...
def test_rejected_call_can_be_corrected_and_failed_final_drops_proposals() -> None:
    ...
    first = asyncio.run(generate_recipe_turn(store, created.session_id, generator))
    retry = asyncio.run(generate_recipe_turn(store, created.session_id, generator))
    ...
    next_turn = asyncio.run(
        generate_recipe_turn(store, created.session_id, ScriptedGenerator([inspect_recovered_context]))
    )
...
def test_active_turn_is_busy_but_independent_session_can_complete() -> None:
    ...
    async def run() -> None:
        first_task = asyncio.create_task(generate_recipe_turn(store, first.session_id, BlockingGenerator()))
        await entered.wait()
        busy = await generate_recipe_turn(store, first.session_id, ScriptedGenerator([final()]))
        independent = await generate_recipe_turn(store, second.session_id, ScriptedGenerator([final()]))
...
def test_cancellation_after_registration_releases_turn_and_drops_proposal() -> None:
    ...
    async def run() -> None:
        task = asyncio.create_task(generate_recipe_turn(store, created.session_id, BlockingGenerator()))
        await entered.wait()
        task.cancel()
